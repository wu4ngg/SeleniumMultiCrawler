...
# driver.get(f'https://www.lazada.vn/catalog/?q={encodedQuery}')
# element_prodname = driver.find_elements(By.CSS_SELECTOR, 'div.RfADt > a')
# element_price = driver.find_elements(By.CLASS_NAME, 'ooOxS')
# products = []
# for name, price in zip(element_prodname, element_price):
...
# print(products)
# df_prods = pd.DataFrame(products)
# new_row = {'prod_name': 'New Product', 'prod_price': '1000', 'prod_link': 'http://example.com'}
# df_prods = df_prods.append(new_row, ignore_index=True)
# df_prods.to_excel('dell_xps_13.xlsx', index=False)
# df_prods = pd.DataFrame(columns=['prod_name', 'prod_price', 'prod_link'])
# with pd.ExcelWriter('dell_xps_13.xlsx', mode='a', engine='openpyxl') as writer:
#     df_prods.to_excel(writer, sheet_name='NewSheet', index=False)
# driver.quit()
